...
% 
% ry=rr*cos(2*Tphi+Talpha)+ix-rr*cos(Talpha);
% rx=rr*sin(Tphi+Talpha)+iy-rr*sin(Talpha);
% rz=iz;
%    
% dry=-2*rr*sin(2*Tphi+Talpha)*phiDot;
% drx=rr*cos(Tphi+Talpha)*phiDot;
% drz=0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%C Path
   phi_sin=2*pi*sin(0.5*pi*t/T);
...
qDot=pinv(J)*(dr_xl+gamma*(r_xl-f_xl));% CRNN

%qDot=gamma*J'*(r_xl-f_xl);% GRNN
